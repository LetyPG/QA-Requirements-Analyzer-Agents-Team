...
The framework is designed as an **AI EngineeringOps** solution that introduces a **Shift Left Quality** approach through a team of specialized AI Agents for Requirements Refinement Analysis.The framework orchestrates the analysis process, but the final responsibility for requirement approval, prioritization, and quality decisions remains with the human team. Its goal is to increase refinement efficiency, expose hidden quality concerns early, and support QA professionals, analysts, and product teams in producing implementation-ready requirements.

Think of this as an LLM narrow capability that enhances and conditions its reasoning through specific parameters and knowledge bases, reproducing the intellectual human process as a Software QA Specialist on Requirement Analysis; this increases deterministic validation, reduces hallucinations and variability of the solution process, in consequence the response reliability and consistency, and is finally augmented by human oversight and decision-making authority.

## INDEX

| INDEX | description |
|---|---|
| [What Problem Does It Solve?](#what-problem-does-it-solve) | Describe a list of common problems in projects caused by a lack of strong quality assurance practices in the early development cycle during the planning and analysis phases  |
...
```text
|_agents/                  # sub-agents capabilities and their skills and knowledge base/RAG components
|_artifacts/               # Storage for the assessment results of sub-agents validations executed by the orchestrator agent, including log trace.
|_assets/                  # Static Schema used for artifact validations as contract compliance, and others for final report generation
|_data-test-poc/           # Data test for testing the solution
...
|_project-context/         # Project context documentation used as a persistence memory
|_skills/                  # Specific orchestrator procedures packages as skill definitions.
...
- Init a chat prompt in the IDE or via CLI, request the refinement process with the `orchestrator-agent.md`. 
- Use this specific command instruction always **Run**, followed by the input artifacts (RF and US). If you don't follow this format setup as uniqueness, the `PreToolUse hook` will block you; this was implemented as a security measure; see more details in [Architecture Details-Hook Section](./docs/architecture_implementation_details_roadmap.md#hook-layer) and [PreToolUse Hooks](./hooks/README.md).
...
For better practice and usage, see:
...
The project uses 2 important artifacts considered as constants for some reasoning and persistence contextual memory; this must be adapted to the real project context, for example:
...
>For more details about these setup requirements, see the file [user_guide.md](./docs/user_guide.md#user-setup-responsabilities--configuration-notes).
...
|---|---|---|
|`PreToolUse` Hooks | Scripts | Execute Scripts before agent tool calls for workflow execution; control security checks in a deterministic way to prevent command and prompt injection, also validates user prompt language to guarantee consistency between user request language and artifact outputs, providing better UX. For more details, see [Hook Details](./hooks/README.md).|
