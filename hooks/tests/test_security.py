...
class TestCheckExtension:
    ...
    def _blocked(self) -> frozenset[str]:
        return sl.BLOCKED_EXTENSIONS

    def test_exe_is_blocked(self):
        reason = sl._check_extension("payload.exe", self._blocked())
        assert reason is not None
        assert ".exe" in reason

    def test_sh_is_blocked(self):
        reason = sl._check_extension("install.sh", self._blocked())
        assert reason is not None

    def test_py_is_blocked_by_default(self):
        reason = sl._check_extension("script.py", self._blocked())
        assert reason is not None
        assert ".py" in reason

    def test_md_is_allowed(self):
        reason = sl._check_extension("requirements.md", self._blocked())
        assert reason is None

    def test_pdf_is_allowed(self):
        reason = sl._check_extension("spec.pdf", self._blocked())
        assert reason is None

    def test_txt_is_allowed(self):
        reason = sl._check_extension("notes.txt", self._blocked())
        assert reason is None

    def test_compound_extension_pdf_exe_is_blocked(self):
        """Critical: requirements.pdf.exe must be caught via pathlib.suffixes."""
        reason = sl._check_extension("requirements.pdf.exe", self._blocked())
    ...
    def test_compound_extension_tar_gz_is_allowed(self):
        ...
        reason = sl._check_extension("data.tar.gz", self._blocked())
        assert reason is None
    ...
    def test_py_allowed_when_overridden(self):
        ...
        effective = sl._effective_blocked_extensions(ext)
        reason = sl._check_extension("script.py", effective)
        assert reason is None

    def test_dll_is_blocked(self):
        reason = sl._check_extension("library.dll", self._blocked())
        assert reason is not None

    def test_docm_is_blocked(self):
        reason = sl._check_extension("report.docm", self._blocked())
        assert reason is not None

    def test_no_extension_file_is_allowed(self):
        """A plain filename with no extension should not be blocked."""
        reason = sl._check_extension("Makefile", self._blocked())
        assert reason is None
...
class TestCheckFileSize:
    ...
    def test_file_over_limit_is_blocked(self):
        ...
        with patch("hooks.security_language.Path") as MockPath:
            ...
            reason = sl._check_file_size("bigfile.md", limit)
        assert reason is not None
    ...
    def test_file_under_limit_is_allowed(self):
        ...
        with patch("hooks.security_language.Path") as MockPath:
            ...
            MockPath.return_value = mock_path_instance
            reason = sl._check_file_size("small.md", limit)
        assert reason is None
    ...
    def test_nonexistent_file_is_skipped(self):
        ...
        with patch("hooks.security_language.Path") as MockPath:
            ...
            MockPath.return_value = mock_path_instance
            reason = sl._check_file_size("ghost.md", 10 * 1024 * 1024)
        assert reason is None
...
class TestRunSecurityGate:
    ...
    def test_no_files_passes_gate(self):
        ...
        result = sl.run_security_gate(ctx)
    ...
    def test_safe_file_passes_gate(self):
        ...
        with patch.object(sl, "_check_mime_type", return_value=None), \
             patch.object(sl, "_check_file_size", return_value=None):
            result = sl.run_security_gate(ctx)
...
class TestPromptInjectionGate:
    ...
    def test_empty_prompt_is_blocked(self):
        reason, extracted = sl._check_prompt_injection("")
        assert reason is not None
