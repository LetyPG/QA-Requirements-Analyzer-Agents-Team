...
This document lists the risks identified during the implementation phase of the framework. Consider that this list is generated as part of the solution design process, and must be used as reference to identify and mitigate risks during solution development and usage. This list is not exhaustive and is subject to change.
...
| [Pending Risks Mitigations Recommendations](#pending-risks-mitigations-recomendations) | Description of all risks classified as Pending: R1, R2, R3, R4, R5, R6 | 
...
| [R13 — No end-to-end workflow correlation](#r13) | Medium | Low (workflow_run_id) | Pending but controlled by keeping the same trace_id for the same run (by default the RF-id is used as trace_id)  |
| [R2 — Single context file bottleneck](#r2) | Medium | High (multi-file context requires architecture change) | Pending | 
| [R3 — Risk calibration drift](#r3) | Medium | Low (onboarding checklist + manifest metadata) | Pending- Controls depend on user configurations | 
...
| [R14 — Security prompt blocks](#r14) | Low-Medium | Low (manually review and edit prompts) | Accepted |
| [R15 — Skills Quality Degradation and Reliability Drop](#r15) | Medium | Medium skills observability and maintenance plan | Pending but controlled by subagent evaluation |

## **Pending** Risks Mitigations Recommendations

Some mitigation strategies rely on user configurations and practices, these are classified as Pending and are documented in the [User Guide and Best Practices](../docs/user_guide.md#user-setup-responsabilities--configuration-notes).
...
Also in case of any component modifications (not recommended), or improvements on the agents logic, it must be done carefully and after understanding and having clarity about the real impact on the whole workflow, how it affects the dependencies, and the correct functioning of all the system.   
...
```
- Find coincident section to add your real information in the sections provided, this will be considered as the main and relevant context information that the agents will consume.
- Other information that is not too relevant, append in new sections after the existing ones, and describe clearly this new context information.

>Keep relevant information only, do not overload the manifesto with irrelevant information, the goal is to provide context information to the agents that helps them to understand the project and make better decisions. Avoid high consumption of resources with your own decisions.

**Mitigation Roadmap as High maturity implementation:**
...
**Mitigation Recommendations:**
- See the [Reference Risk Standard](../agents/risk-evaluator-qa-strategy-agent/skills/risk-evaluator/reference/reference_risk_standard.md) file for editing with your real project information.
...
### R15 
**Skills Quality Degradation and Reliability Drop**

This risk is directly related to system observability and maintenance. 
The risk of skills quality degradation is directly related to sub-agent LLM provider drift, which could cause significant quality degradation and reliability drop if the skill prompt is not adjusted to the new LLM version, provider parameters, or system scaling changes. 

**Mitigation:**
-  This framework needs to use an external layer or dedicated agent, as a skills evaluator, using the LLM-as-judge methodology.
-  Create a plan to evaluate the quality of the skills and the reliability drop, with golden dataset and evals, as mentioned in the [evals section](../docs/qa_criteria_test_components_strategies.md#evals) in each skill or dedicated subagent . This is the most important step to mitigate this risk and ensure the quality of the framework.
...
In the first implementation version, `risk.log` was used as an append-only text file maintained by an LLM. As different LLM versions, different prompt iterations, or different users contribute entries over time, the format drifts. The "historical consistency check" becomes pattern matching against noisy, inconsistent entries. This is a known problem in ADR (Architectural Decision Record) systems that lack enforced structure.

**Mitigation:** 
- Implemented the transition of the risk evaluation storage system from `risk.log` to `risk.jsonl`, a JSON Lines format with a schema validated on every write. This is a low-effort change that makes the log both machine-readable and auditable without changing the append-only semantic.
...
- Added `timestamp` for all files generated as a naming convention.
- Defined as a rule and verification check in all sub-agents to include the same `timestamp` in their output artifacts, e.g: `outputs/final_report_RF-001_20260717_162106.md`
- Also the final report artifacts include the `trace_id` and the `timestamp`, which ensures the traceability and the audit trail of the process.
...
**Mitigation:** 
- Added `last_updated` and `version` metadata blocks to the Manifesto. The Orchestrator reads this and warns (doesn't block) the user if the file hasn't been updated in more than a configurable number of days. This is a documentation-level change, not a code change.
...
The `risk-evaluator-qa-strategy-agent` runs at a temperature different from `0.0`. Since the agent determines the business impact and technical complexity scores that feed the deterministic Python calculator script, any non-zero temperature can introduce variance in the risk evaluation, which also directly impacts the strategy recommendations. The `risk_calculator.py` script is deterministic, but the LLM *determines the inputs*: the Business Impact and Technical Complexity scores. For example: 
...
**Mitigation:** 
- Currently traceability is controlled using a `trace_id` generated by the hook system, but this is not a `workflow_run_id` that links the Orchestrator, Agent A, Agent B, and all their artifacts into a single traceable execution.
...
### R14
**Security prompt blocks**

Currently the security `Hook` allows a natural user instruction to start the interaction like `Requirements Refinement Request` at the beginning of the prompt and accepts as input an IEEE format `RF-[ID]: The system must [action]` and `US[as a [role] I want to [goal] so that [reason]]`. 
...
**Accepted:** 
The current security prompt block is not robust enough and can be bypassed by a crafted prompt. Despite this, the current setup accepts these risks because the system is agnostic and highly generalized, with numerous ways to perform validations as objectively as possible. Consequently, the risk of potential prompt injection is accepted, even with the existing protection layer, and is further mitigated by a second layer involving LLM constraints defined within the system prompt.

**Mitigation Recommendations:**
It is recommended to use an exact instruction format to avoid unexpected blocks, as follows: 
